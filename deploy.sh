...
apt-get install -y git nginx mysql-server python3-pip redis-server apache2-utils
pip3 install jinja2 flask gunicorn gevent pymysql flask_sqlalchemy flask_admin flask_mail marrow.mailer redis Celery pyquery pillow
...
cp switch_home.nginx /etc/nginx/sites-enabled/switch_home

# systemd python 服务, 消息队列
cp switch_home.service /etc/systemd/system/switch_home.service
cp switch_home-message-queue.service /etc/systemd/system/switch_home-message-queue.service

# 重启服务器
systemctl daemon-reload
systemctl restart switch_home
